                   ...
                   (case op
                     (:implies
                      (destructuring-bind (a b) args
                        ;; TODO: for implies case
			(assert (and args (null (cdr (cdr args)))))
			(visit (exp-op 'or b (cons 'not (list a))) truth)
                        ))
                     ...
                     (:iff
                      (destructuring-bind (a b) args
                        ;; TODO: for iff case
				(assert (and args (null (cdr (cdr args)))))
			(visit (exp-op 'and (exp-op ':implies a b) (exp-op ':implies b a)) truth)
                        ))
                     ...
                     (and
                      ;; TODO: for 'and' case
		      (if truth
			  `(and ,@(loop for l in args collect (visit l truth)))
			  `(or ,@(loop for l in args collect (visit `(not ,l) (not truth))))))
                      
                     (or
                      ;; TODO: for 'or'  case
		       (if truth
			  `(or ,@(loop for l in args collect (visit l truth)))
			  `(and ,@(loop for l in args collect (visit `(not ,l) (not truth))))))
...
(defun %dist-or-and-1 (literals and-exp)
  ...
  (cons 'and (loop for l in (cdr and-exp)
		  collect (exp-op 'or `(or ,@literals) l)))
  )
...
(defun %dist-or-and-and (and-exp-1 and-exp-2)
  ...
  (let ((exp nil))
    (loop for l1 in (cdr and-exp-1)
       do(loop for l2 in (cdr and-exp-2)
	    do (push (exp-op 'or l1 l2) exp)))
    (push 'and exp))
    )
           ...
           (rec (rest)
             (if (consp rest)
                 (destructuring-bind (x &rest rest) rest
                   (if (maxterm-unit-p x)
                       (progn
			 (if (null (maxterm-pos x)) (repeat (car (maxterm-neg x)) nil)
...
(defun dpll-choose-literal-optimal (maxterms)
    (let ((flist ()) (lit nil) (value (1+ (length maxterms))))
    	(loop for term in maxterms do
    		(loop for temp in (maxterm-pos term)
    			if (null (assoc temp flist))
    			do (setf flist (cons (cons temp 1) flist))
    			else do (setf (cdr (assoc temp flist))
    				(1+ (cdr (assoc temp flist)))))
    		(loop for temp in (maxterm-neg term) if (null (assoc temp flist)) do (setf flist (cons (cons temp 1) flist))
    			else do (setf (cdr (assoc temp flist))
    				(1+ (cdr (assoc temp flist))))))
    	(loop for term in flist if (< (cdr term) value) do (setf lit (car term)) (setf value (cdr term)))
    	(values lit)))
...
(defun dpll (maxterms)
	...
	(rec (maxterms bindings)
		(if (maxterm-result maxterms)
			(values maxterms bindings)
			(progn
				(let* ((propagate-result (multiple-value-list (dpll-unit-propagate maxterms bindings)))
					(newterms (first propagate-result))
					(newbindings (second propagate-result)))
				(cond 
					((null newterms) (values newterms newbindings))
					((eq newterms :UNSAT) (values newterms newbindings))
					(t
						(let* ((variable (dpll-choose-literal newterms))
							(positive (make-maxterm :pos (list variable) :neg NIL))
							(negative (make-maxterm :pos NIL :neg (list variable)))
							(positive-result (multiple-value-list (rec (cons positive newterms) newbindings)))
							(negative-result (multiple-value-list (rec (cons negative newterms) newbindings))))
						(cond
							((null (first positive-result)) (values (first positive-result) (second positive-result)))
							(t (values (first negative-result) (second negative-result))))))))))))
		...
		(if (maxterm-result maxterms)
			...
			(progn
				...
				(cond 
					...
					((eq newterms :UNSAT) (values newterms newbindings))
					(t
						(let* ((variable (dpll-choose-literal-optimal newterms))
							(positive (make-maxterm :pos (list variable) :neg NIL))
							(negative (make-maxterm :pos NIL :neg (list variable)))
							(positive-result (multiple-value-list (rec (cons positive newterms) newbindings)))
							(negative-result (multiple-value-list (rec (cons negative newterms) newbindings))))
...
(defun sat-p-optimal (e)
  "Check satisfiability of e."
  (multiple-value-bind (is-sat bindings)
      (dpll-optimal (cnf-maxterms (exp->cnf e)))
    (when is-sat
      (assert (exp-eval e bindings)))
    (values is-sat bindings)))
